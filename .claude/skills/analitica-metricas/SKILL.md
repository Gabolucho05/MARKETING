...
Usa `references/reporte-de-metricas.md` para traducir los números en la decisión concreta que motivó la medición en primer lugar: fórmulas de ROAS/ROAS ciego/MER/ROI/CAC/LTV, consolidación multi-canal, modelo de atribución declarado, invalidadores de rendimiento, y las dos cadencias de dashboard (semanal operativo / mensual estratégico). Esto conecta con `market-report` / `market-report-pdf` si el usuario quiere el entregable formal para un cliente, con `dataviz` para los gráficos, y con los invalidadores definidos en `paid-media` y `estrategia-planificacion`.
